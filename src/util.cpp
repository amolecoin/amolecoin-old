...
const char * const BITCOIN_CONF_FILENAME = "amolecoin.conf";
const char * const BITCOIN_PID_FILENAME = "amolecoin.pid";
...
#else
    const char* pszModule = "amolecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "amolecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/amolecoin";
#else
    // Unix
    return pathRet / ".amolecoin";
#endif
#endif
}
